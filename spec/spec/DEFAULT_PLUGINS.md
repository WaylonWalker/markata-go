...
**Purpose:** Generate nginx-native redirects plus HTML fallback pages from a `_redirects` file.
...
- An optional redirect status code may follow the destination (`301`, `302`, `307`, `308`)
- Both paths must start with `/`
- Destination may also be an absolute `http://` or `https://` URL
...
3. Filter out redirect sources that collide with an existing file path in output (for example `rss.xml`)
4. Generate `{output_dir}/redirects.conf` with exact-match nginx `location` blocks
5. If `generate_html_fallback` is enabled, create `{output_dir}/{original_path}/index.html` with meta refresh redirect
6. Cache results based on redirect content, template content, and redirect plugin config

**Output:**
The plugin generates `{output_dir}/redirects.conf`:

```nginx
# Generated by markata-go redirects plugin.
# Include this file inside an nginx server block.
location = /old-path {
    return 301 /new-path;
}
```

When `generate_html_fallback = true`, it also creates `{output_dir}/{original_path}/index.html`:
...
# Parse, filter, and generate nginx config
for redirect in parse_redirects(content):
    if redirect_conflicts_with_existing_output(redirect):
        continue
    append_to_nginx_conf(redirect)
    if config.generate_html_fallback:
...
# Custom nginx include path under output/
[markata-go.redirects]
