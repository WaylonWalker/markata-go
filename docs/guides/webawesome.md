...
When `source = "vendor"`, the stylesheet and autoloader are served from your site under `output_dir` (for example `/assets/vendor/webawesome/styles/webawesome.css` and `/assets/vendor/webawesome/webawesome.loader.js`). These files come from the npm tarball downloaded through the shared `[markata-go.assets]` cache/output pipeline. When `source = "cdn"`, they are served from `https://cdn.jsdelivr.net/npm/@awesome.me/webawesome@VERSION/dist/`.
