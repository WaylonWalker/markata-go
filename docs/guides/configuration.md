...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `true` | Enable/disable embeds plugin |
| `internal_card_class` | string | `"embed-card"` | CSS class for internal embed cards |
...
The default theme adds a small preview when a local navigation URL matches a generated feed or published post. For a feed such as `/blog/`, hovering the link or focusing it with a keyboard shows its description, public post count, total words, reading time, and publication rhythm when those values are available. For a post, the preview uses its description, tags, word count, and reading time. External and unmatched links stay as ordinary links. Previews are hidden on narrow or touch screens, where the navigation is a scrollable strip.
