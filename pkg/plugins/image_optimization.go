...
func buildPictureSources(src string, formats []string, widths []int, sizes string) []string {
	...
	for _, format := range formats {
		...
		if sizes == "" {
			sources = append(sources, fmt.Sprintf(`<source type="%s" srcset="%s">`, mime, strings.Join(items, ", ")))
			continue
		}
		sources = append(sources, fmt.Sprintf(`<source type="%s" srcset="%s" sizes="%s">`, mime, strings.Join(items, ", "), sizes))
	}
	return sources
}
...
func imageOptimizationCachePath(cacheDir, sourcePath, format string, width, quality int, encoder string) string {
	...
	hasher.Write([]byte("|"))
	hasher.Write([]byte(fmt.Sprintf("%d", width)))
	hasher.Write([]byte("|"))
