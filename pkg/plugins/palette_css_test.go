...
func TestPaletteCSSPlugin_Write_NoPaletteSkips(t *testing.T) {
	...
	if _, err := os.Stat(cssPath); err == nil {
		t.Error("expected variables.css to NOT be created when no palette is configured")
	}
}
...
func TestPaletteCSSPlugin_FullBuildFlow(t *testing.T) {
	...
	m.SetAssetHash("css/variables.css", "deadbeef")
	...
	hashedPath := filepath.Join(tmpDir, "css", "variables.deadbeef.css")
	if _, err := os.Stat(hashedPath); err != nil {
		t.Fatalf("expected hashed variables.css to exist: %v", err)
	}
}
