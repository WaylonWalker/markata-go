...
// Regular-file modification times are preserved so cache decisions see the same
// fixture state in every clean comparison workspace.
func CopyFixture(src, dst string) error {
	...
	if sourceInfo.Mode()&os.ModeSymlink != 0 {
		return fmt.Errorf("source is a symlink: %q", src)
	}
	if !sourceInfo.IsDir() {
	...
	if info, err := os.Lstat(dst); err == nil {
		...
		if !info.IsDir() {
			return fmt.Errorf("destination is not a directory: %q", dst)
		}
...
func copyDir(src, dst string, excluded []string) error {
	return filepath.Walk(src, func(path string, info os.FileInfo, err error) error {
		...
		if shouldExcludeFixturePath(rel, excluded) || (excluded != nil && isBuildMetadataPath(rel)) {
			if info.IsDir() {
		...
		if e == nil {
			e = os.Chtimes(target, info.ModTime(), info.ModTime())
		}
		return e
	})
}
...
func shouldExcludeFixturePath(path string, excluded []string) bool {
	...
	for _, excludedPath := range excluded {
		...
		// Root build-state directories (for example output and cache) are
		// excluded only at the fixture root. A content file named
		// content/output/example.md is valid source input and must not be
		// mistaken for the site's generated output tree. Explicit nested
		// exclusions, such as assets/vendor, still match their descendants.
		if filepath.Dir(cleanExcluded) == "." {
			firstPathPart := cleanPath
			if separator := string(filepath.Separator); strings.Contains(cleanPath, separator) {
				firstPathPart = strings.SplitN(cleanPath, separator, 2)[0]
			}
			if firstPathPart != cleanExcluded {
				continue
			}
		}
