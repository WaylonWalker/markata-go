...
func TestFontpackConfigLoadsAcrossFormats(t *testing.T) {
	tests := []struct {
		...
		{"toml", FormatTOML, "[markata-go]\nfontpack = \"field-notebook\"\nfontpacks_file = \"config/fonts/catalog.yaml\"\n"},
		{"yaml", FormatYAML, "markata-go:\n  fontpack: field-notebook\n  fontpacks_file: config/fonts/catalog.yaml\n"},
		{"json", FormatJSON, `{"markata-go":{"fontpack":"field-notebook","fontpacks_file":"config/fonts/catalog.yaml"}}`},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if cfg.FontpacksFile != "config/fonts/catalog.yaml" {
				t.Fatalf("FontpacksFile = %q", cfg.FontpacksFile)
			}
		})
	}
}
