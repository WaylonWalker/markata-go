...
| "Sidenotes / margin notes" | Use standard footnotes `[^1]`. They become margin notes on wide screens and tap popovers on narrow ones; the generated copy is visual-only and the canonical footnote list is exposed once to assistive technology. | Add HTML asides or a plugin |
...
| "Reader mode" | Exists: header button, `s` shortcut, `?reader=1`. Same-origin keyboard, series, feed-sidebar, and view-transition navigation preserve `?reader=1`. Hide the button with `components.post_meta.reader_toggle = false`. | Build a separate template |
